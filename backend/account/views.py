...
#for creating a token manually during signup and login
  ...
  def post(self, request, format=None):
    ...
    if serializer.is_valid(raise_exception=True):
        ...
        if user is not None:
            ...
            return Response({'errors':{'non_field_errors':['please check your credentials']}}, status=status.HTTP_404_NOT_FOUND)
...
class PasswordResetView(APIView):
    def post(self, request, format=None):
        ...
        if serializer.is_valid(raise_exception=True):
            return Response({'msg': 'Password Reset link sent to email. Please check your email'}, status=status.HTTP_200_OK)
